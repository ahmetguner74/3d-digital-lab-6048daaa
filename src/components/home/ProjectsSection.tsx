    ...
    const fetchFeaturedProject = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('projects').select('id, title, description, slug, cover_image').eq('featured', true).eq('status', 'Yayında').order('updated_at', {
          ascending: false
        }).limit(1).single();
        if (error) {
          ...
          <div className="relative w-full reveal">
            <Link to={`/projects/${featuredProject.slug}`} className="block">
              <div className="relative aspect-video w-full rounded-lg overflow-hidden bg-muted cursor-pointer">
                ...
                {featuredProject.cover_image ? <img src={featuredProject.cover_image} alt={featuredProject.title} className="w-full h-full object-cover" /> : <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/10">
